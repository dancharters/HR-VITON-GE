...
INPUT_SIZE = 512  # input image size for Generator -> Can handle multiple sizes, no need to change this
...
def pre_process(raw_img, raw_mask, multiple):
    ...
    # resize raw image & mask to designated size
...
def inpaint(raw_img, 
    ...
    # post-processing
    res_raw_size = post_process(raw_img, img_large, mask_large, inpainted_512[0], img_512, mask_512[0], attention[0], multiple)
...
def read_imgs_masks(args):
    ...
    print('Number of garment images: ' + str(len(paths_img)))
    print('Number of mask images: ' + str(len(paths_mask)))
    # print(paths_img)
    # print(paths_mask)
...
def main():
    ...
    # args.images = '../../../data/test/inpainting_steps/extracted_garment'  # input image directory
    # args.masks = '../../../data/test/inpainting_steps/inpainting_masks'  # input mask director
    # args.output_dir = '../../../data/test/cloth'  # output directory
    args.multiple = 6  # multiples of image resizing

    paths_img, paths_mask = read_imgs_masks(args)

    # Get counter for number of images left
    n = len(paths_img)

    if not os.path.exists(args.output_dir):
        os.makedirs(args.output_dir)
    with tf.Graph().as_default():
        with open('hifill.pb', "rb") as f:
        ...
        with tf.compat.v1.Session() as sess:
            ...
            for path_img, path_mask in zip(paths_img, paths_mask):
                ...
                inpainted = inpaint(raw_img, raw_mask, sess, inpainted_512_node, attention_node, mask_512_node,
                                    image_ph, mask_ph, args.multiple)
                filename = args.output_dir + '/' + os.path.basename(path_img)
                # Remove the .png part and the garment_
                filename = filename.replace(".png", "")
                filename = filename.replace("garment_", "")
                cv2.imwrite(filename + '_inpainted.jpg', inpainted)
                n -= 1
                print(f"{n} images left")
    print("Inpainting complete for all images!")
