...
def extract_garment_region(raw_image_path, mask_image_path, mask_output_path):
    ...
    # # Add padding to get the desired size of 1024x1024
    # desired_size = (1024, 1024)  # Desired size (height, width)
    # top, bottom = 0, 0  # No padding on top and bottom
    # left, right = (desired_size[0] - garment_region.shape[1]) // 2, (desired_size[0] - garment_region.shape[1]) // 2  # Add padding at the sides
    # color = [255, 255, 255, 0]  # White color, fully transparent
    # garment_region = cv2.copyMakeBorder(garment_region, top, bottom, left, right, cv2.BORDER_CONSTANT, value=color)
...
def main():
    raw_images_dir = '../../../data/test/image'
    masks_dir = '../../../data/test/image-parse-v3'
    output_dir = '../../../data/test/inpainting_steps/extracted_garment'
    mask_output_dir = '../../../data/test/inpainting_steps/extracted_garment_mask'
