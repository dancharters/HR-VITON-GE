...
def get_opt():
	...
	parser.add_argument('--predict_dir', default='../../../data/results/output_images/with_gt_garments_paired')
...
def Evaluation(opt, pred_list, gt_list):
	...
	model.eval()
	inception_model = inception_v3(pretrained=True, transform_input=False).type(torch.FloatTensor)
	...
	with torch.no_grad():
		...
		for i, img_pred in enumerate(pred_list):
			...
			# Calculate LPIPS
			gt_img_LPIPS = T2(gt_img).unsqueeze(0)
			pred_img_LPIPS = T2(pred_img).unsqueeze(0)
			...
			pred_img_IS = T3(pred_img).unsqueeze(0)
			preds[i] = F.softmax(inception_model(pred_img_IS)).data.cpu().numpy()

			gt_img_MSE = T1(gt_img).unsqueeze(0)
			pred_img_MSE = T1(pred_img).unsqueeze(0)
