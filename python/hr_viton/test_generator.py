...
def get_opt():
    ...
    parser.add_argument('--test_name', type=str, default='with_extracted_garments', help='name for test')
    parser.add_argument("--dataroot", default="../../../data")
    parser.add_argument("--datamode", default="test")
    parser.add_argument("--data_list", default="test/test_pairs.txt")
    parser.add_argument("--output_dir", type=str, default="../../../data/results/output_images/with_extracted_garments")
    ...
    parser.add_argument('--checkpoint_dir', type=str, default='checkpoints/info', help='save checkpoint infos')
    parser.add_argument('--tocg_checkpoint', type=str, default='checkpoints/mtviton.pth', help='tocg checkpoint')
    parser.add_argument('--gen_checkpoint', type=str, default='checkpoints/gen.pth', help='G checkpoint')
    ...
    parser.add_argument('--occlusion', type=lambda x: (str(x).lower() == 'true'), default=True, help="Occlusion handling")
...
def test(opt, test_loader, tocg, generator):
    ...
    if opt.output_dir is not None:
        ...
        output_dir = os.path.join('../../../data/results/output_images', opt.test_name, opt.datamode, opt.datasetting, 'generator', 'output')
    grid_dir = os.path.join('../../../data/results/output_process', opt.test_name, opt.datamode, opt.datasetting, 'generator', 'grid')
    ...
    # num = 0
    images_left = len(test_loader.data_loader) * opt.batch_size
    print(f"{images_left} images to process\n")
    ...
    with torch.no_grad():
        for inputs in test_loader.data_loader:
            ...
            # print(num)
            images_left -= 1
            print(f"Images left to process: {images_left}")

    # Calculate elapsed time for testing
    elapsed_seconds = round(time.time() - iter_start_time)
    # Convert to timedelta
    elapsed_time = timedelta(seconds=elapsed_seconds)
    # Print in HH:MM:SS format
    print(f"\nTest time: {str(elapsed_time)}\n")


def main():
    opt = get_opt()
    print(opt)
    print(f"Starting test '{opt.test_name}'")
    if opt.cuda:
        print(f"Using GPUs with IDs {opt.gpu_ids}")
    else:
        print("Using CPU")
    ...
    print(f"Completed test '{opt.test_name}'")
