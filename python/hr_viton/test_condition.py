...
def get_opt():
    ...
    parser.add_argument('--D_checkpoint', type=str, default='checkpoints/discriminator_mtviton.pth', help='D checkpoint')
...
def test(opt, test_loader, board, tocg, D=None):
    ...
    for inputs in test_loader.data_loader:
        if opt.cuda:
            ...
            cm_paired = torch.FloatTensor((cm_paired.detach().cpu().numpy() > 0.5).astype(float))
            # input2
